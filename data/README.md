...
| `ReefWideBRUVUVC.csv` | Reef-aggregated table with DAG-friendly column names (latitude, island_geomorphology, lagoon_size, emerged_land_area, pop_dens, ave_npp, ave_temp, transient_pelagic_sharks, sicklefin_lemon_sharks, reef_sharks, piscivores, herbivores, invertivores, planktivores, relief, other_algae, hard_coral, crustose_coraline_algae, invert). Used by `04_DAG_consistency.R` and the SST/reef-sharks lm in `08_supplementary_plots.R`. | NFF |
| `FPDAG_match_table.csv` | Legacy column-name lookup mapping `name_in_data` to `name_in_dag`. Mostly identity for the current data; retained for compatibility with the legacy DAG-consistency workflow. | Project |
