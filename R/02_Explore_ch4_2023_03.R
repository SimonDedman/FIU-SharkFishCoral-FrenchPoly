...
benthic.reef.df <- benthic.raw |>
  ...
  dplyr::select(
    reef_name, Relief, Sand, Rubble, Pavement, Other.Algae, Hard.Coral, CCA,
    Soft.Coral, Invert, chi_benthos_percent, n_survey)
...
prey.uvc.survey.sum.df <- fish.uvc.raw |>
  ...
  dplyr::mutate(dplyr::across(contains("biomass_g_per_m2"),
                              \(x) x/500,
...
pred.bruv.df1 <- elasmo.bruv.raw |>
  ...
  dplyr::select(reef_name:topo, latitude, longitude, lagoon.size, Emerged.land.area,
...
survey.wide.df1 <- benthic.raw |> # 167 x 19
  ...
  merge(pred.bruv.df1 |> # see directly above for contents
  ...
  mutate(
    ...
    biomass_g_per_m2_Piscivore = pred_tel_biomass_g_per_m2 + biomass_g_per_m2_Piscivore,
    chi_Piscivore_percent = chi_pred_tel_percent + chi_Piscivore_percent,
    .keep = "unused" # remove Piscivore columns
  ) |>
  mutate(pop.dens = Population.size / Emerged.land.area,
         ...
         across(contains("percent"),
                \(x) ((chi_benthos_percent + x) / 2),
                .names = "{str_replace(.col, 'percent', 'score')}"), # replaces percent with score
         # sum all benthos, invert, plank, herb, piscivore
         chi_all_score = rowSums(across(contains("_percent"))),
         # convert scores to grades
         across(contains("_score"), ~ case_when(
           . >= 0.80 ~ "Very Healthy",
           . >= 0.60 ~ "Healthy",
           . >= 0.40 ~ "Fair",
           . >= 0.20 ~ "Degraded",
         ...
         # replace NA with 0
         across(contains(c("biomass_g", "biomass_g_per_m2")), ~ ifelse(is.na(.), 0, .)),
         # convert to factors
...
compare.tel.df1 <- teleost.bruv.raw |>
  ...
  merge(survey.wide.df2 |>
          ...
          group_by(reef_name) |>
          summarise(biomass_g_per_m2_Piscivore = mean(biomass_g_per_m2_Piscivore, na.rm = TRUE)),
        by = c("reef_name")) |> # SD fix for missing biomass_g_per_m2_Piscivore in ggplot call below
