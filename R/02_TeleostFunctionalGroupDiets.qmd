...
Diet <- readr::read_csv(here("NFF_data", "Trophic_Categorisation_Desbiens.csv")) |> 
...
```{r}
#| echo: false
OFGaddition <- readr::read_csv(here("NFF_data", "OfficialFnGp-additions-Coralivores.csv")) |> 
...
write.csv(x = fish.spp.list,
          file = here("NFF_data", "TeleostDietFnGps.csv"),
          row.names = FALSE)
```
...
BRUV <- readr::read_csv(here("NFF_data", "wide.reef.60.ch3.2023.01.csv"))
```
...
write.csv(x = ReefWideBRUVUVC,
          file =  here("NFF_data", "ReefWideBRUVUVC.csv"),
          row.names = FALSE)
write.csv(x = UVC,
          file =  here("NFF_data", "UVC.csv"),
          row.names = FALSE)
write.csv(x = BRUV,
          file =  here("NFF_data", "BRUV.csv"),
          row.names = FALSE)
```
...
BRUV <- readr::read_csv(file = here("NFF_data", "BRUV.csv"))
UVC <- readr::read_csv(file = here("NFF_data", "UVC.csv"))
```
...
```{r}
Benthic <- readr::read_csv(file = here("NFF_data", "bethic_sum_reef_2023_02_26.csv"))
...
ReefWideBRUVUVC |>
  ...
  dplyr::mutate(Transient.pelagic.sharks = Great.hammerhead.shark + Scalloped.hammerhead.shark + Tiger.shark,
                ...
                piscivores = AmbushPiscivore_SppNoMean + PursuitPiscivore_SppNoMean,
                herbivores = Browser_SppNoMean + Grazer_SppNoMean + Scraper_SppNoMean) |> 
...
ReefWideBRUVUVC <- ReefWideBRUVUVC |>
  ...
  dplyr::select(
    ...
    piscivores,
    herbivores,
    Invertivore_SppNoMean,
    Planktivore_SppNoMean,
...
```{r convert columns}
ReefWideBRUVUVC <- ReefWideBRUVUVC |>
dplyr::mutate(island_geomorphology = as.numeric(dplyr::case_match(island_geomorphology, # "open atoll"   "near atoll"   "closed atoll" "high rocky"   "high barrier"
"open atoll" ~ "1", 
"closed atoll" ~ "0.75",
"near atoll" ~ "0.5",
"high barrier" ~ "0.25",
